...
export default function StandaloneCourseManagementPage(): React.JSX.Element {
    ...
    const handleCreateCourse = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setFormError(error?.response?.data?.message || "Could not register this course. Please try again.");
        } finally {
            setSubmitting(false);
        }
    };
                        ...
                        {filteredCourses.length > 0 ? (
                            ...
                            <div className="flex flex-col items-center justify-center gap-3 rounded-2xl border border-dashed border-slate-200 bg-slate-50/50 py-16 text-center">
                                ...
                                <div>
                                    ...
                                    {!search && (
                                        <p className="text-xs text-slate-400 mt-1">Register your first course to get started.</p>
                                    )}
                                </div>
                            </div>
                        )}
                ...
                >
                    ...
                    >
                        <div className="mb-5 flex items-center justify-between">
                            <div>
                                <h3 className="text-base font-bold text-slate-900">Register a course</h3>
                                <p className="text-xs text-slate-500">Add a new course to the system catalog.</p>
                            </div>
                            <button
                                onClick={closeModal}
                        ...
                        <form onSubmit={handleCreateCourse} className="space-y-4">
                            ...
                            <div className="flex justify-end gap-3 pt-2">
                                ...
                                <button
                                    type="submit"
                                    disabled={submitting}
                                    className="inline-flex items-center gap-1.5 rounded-xl bg-indigo-600 px-5 py-2.5 text-xs font-semibold text-white shadow-sm transition hover:bg-indigo-700 disabled:opacity-50"
                                >

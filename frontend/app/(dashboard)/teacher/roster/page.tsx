...
interface CourseScoreSummary {
    courseId: string;
    courseName: string;
    totalEarned: number; // Summation out of 100%
}
...
export default function HomeroomRosterPage() {
    ...
    const [isHomeroomTeacher, setIsHomeroomTeacher] = useState<boolean>(false);

    // 1. Fetch User Details & Sections, filtering explicitly for Homeroom sections assigned to this teacher
    useEffect(() => {
        const initializeHomeroomData = async () => {
            try {
                ...
                // Filter sections where this teacher is designated as the homeroomTeacher (unless Admin)
                const assignedHomeroomSections = sectionsData.filter((sec: any) => {
                ...
                if (assignedHomeroomSections.length > 0 || isAdmin) {
                    setIsHomeroomTeacher(true);
                }

                setAllSections(assignedHomeroomSections);

                // Extract unique academic years and grade levels from homeroom sections
                const yearGradeMap = new Map();
                assignedHomeroomSections.forEach((sec: any) => {
                ...
                console.error("Failed to initialize homeroom roster data", error);
            }
        };

        initializeHomeroomData();
    }, []);
        ...
        const fetchRosterData = async () => {
            try {
                ...
                const res = await Axios({
                    ...summeryApi.getSectionRoster, // Ensure your backend API matches this endpoint or update key accordingly
                    params: {
    ...
    return (
        <div className="p-6 bg-gray-50 min-h-screen space-y-6">
            <div className="flex justify-between items-center">
                <div>
                    <h1 className="text-xl font-semibold text-slate-900 tracking-tight">Homeroom Student Roster Sheet</h1>
                    <p className="text-xs text-gray-500 mt-0.5">Consolidated 100% course evaluations for your homeroom section.</p>
                </div>
            ...
            {!isHomeroomTeacher && availableAcademicYears.length === 0 ? (
                <div className="p-8 text-center bg-amber-50 rounded-lg border border-amber-200 text-amber-800 text-sm">
                    ⚠️ You are not designated as a homeroom teacher for any section. Roster views are restricted to assigned homeroom teachers.
                </div>
            ) : (
                <>
                    {/* Filter Bar */}
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-white p-4 rounded-lg shadow-sm border border-gray-200">
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">Homeroom Section</label>
                            <select
                    ...
                    {!selectedSectionId ? (
                        <div className="p-8 text-center bg-white rounded-lg shadow-sm border border-gray-200 text-gray-500">
                            Please select your homeroom section above to view the student roster.
                        </div>
                    ) : loading ? (
                        ...
                        <div className="bg-white rounded-lg shadow-md p-6 space-y-4">
                            <div className="flex justify-between items-center">
                                <h3 className="text-md font-semibold text-slate-900 tracking-tight">
                                    Section Roster Evaluation ({roster.sectionName})
                                </h3>
                                ...
                                <table className="min-w-full divide-y divide-gray-200">
                                    <thead className="bg-gray-50">
                                        <tr>
                                            ...
                                            {/* Dynamic Course Columns (Each scored out of 100%) */}
